<%
  ...
  def mark(s, linky=True):
    ...
    # NOTE(rag): This is the only changed line so whitespace in docstrings is outputed somewhat reasonably.
    s = markdown.markdown(s.strip(), extensions=extensions, output_format='html5').replace('\n', '<br>').replace(' ', '&nbsp;')
    print s
    return s
